...
[] spawn {
	...
	mav_tuning_nitro_handler = (findDisplay 46) displayAddEventHandler ["KeyDown", {
		...
		if (vehicle player != player AND (driver vehicle player) == player) then {
			...
			// Shift Q (Underglow)
			if ((_this select 2) AND (_this select 1) == 16) then {
	...
	// Give all NPCs their addaction
	{
		...
		if (!isNull _npc) then {
			player addAction ["Enter Malden Customs", {[vehicle player] spawn mav_tuning_fnc_start}, nil, 50, true, false, "", format["(%1 distance2d player) < 10", _npcNameString]];
		};
